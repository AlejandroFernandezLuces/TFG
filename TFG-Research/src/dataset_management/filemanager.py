...
def _get_sensors_list(lines):
    ...
    """
    start_end_index, code_name = \
        _get_tables_index(lines)
    sensor_list = []
...
def _rename_sensor(code, line, dict):
    """
...
def get_csv(path, filename):
    ...
    absolute_path = path + "/" + new_filename + "/"

    if not os.path.exists(absolute_path):
        os.mkdir(absolute_path)
    lines = _open_doc(path, filename)
    sensor_list = _get_sensors_list(lines)
